...
function* Capture({ styleActive, styleActiveHover, styleHover, stylePlaceholder, ...props }, { block, custom, debug, indent, index }) {
  const uses = []
  let nextIndex = index + 1

  yield `${indent}<input`
  const { accessed, hasProps } = yield* morphProps(props, { block, debug, indent: `${indent}  `, index })
