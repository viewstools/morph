    ...
    case SET: {
      if (process.env.NODE_ENV === 'development') {
        ...
        if (!flow.has(action.id)) {
          console.error({ type: 'views/flow/invalid-story', id: action.id, availableStories: flow })
          return state
        }
      }
    ...
    <Context.Provider value={context}>
      <ViewsTools
        ...
        onFlowMapChange={next => {
          flow = new Map(JSON.parse(next).map(([key, value]) => [key, {...value, stories: new Set(value.stories) }]))
        }}
      >
        {props.children}
      </ViewsTools>
