...
export default function* List({ blocks, helpers, index:listIndex='i', from:rawFrom, style, tag='View', variable='item' }, { block, custom, debug, indent, index }) {
  ...
  if (tag !== false) {
    ...
    const { accessed:accessedProps, hasProps } = yield* morphProps(props, { block, debug, indent: internalIndent, index })
    if (hasProps) {
      yield indent
