...
import * as $_____lib_event_emitter from '../lib/event_emitter'
import * as $_____lib_dom from '../lib/dom'
import * as $_____lib_oop from '../lib/oop'
import $_____lib_keys from '../lib/keys'
import * as $_____lib_event from '../lib/event'
import * as $_____search from '../search'
import * as $_____lib_useragent from '../lib/useragent'
import * as $_____search_highlight from '../search_highlight'
import * as $_____commands_multi_select_commands from '../commands/multi_select_commands'
import * as $_____mode_text from '../mode/text'
import * as $_____multi_select from '../multi_select'
...
var EventEmitter = $_____lib_event_emitter.EventEmitter
var dom = $_____lib_dom
var oop = $_____lib_oop
var KEYS = $_____lib_keys
var event = $_____lib_event
var Search = $_____search.Search
var useragent = $_____lib_useragent
var SearchHighlight = $_____search_highlight.SearchHighlight
var multiSelectCommands = $_____commands_multi_select_commands
var TextModeTokenRe = $_____mode_text.Mode.prototype.tokenRe
$_____multi_select
...
;(function() {
  ...
  this.getCursor = function(p) {
    ...
    var pos = p == 'anchor'
      ? sel.isEmpty() ? sel.lead : sel.anchor
...
var exCommands = {
  ...
  sort: function(cm, params) {
    ...
    var numberRegex = number == 'decimal'
      ? /(-?)([\d]+)/
      : number == 'hex'
        ? /(-?)(?:0x)?([0-9a-f]+)/i
        : number == 'octal' ? /([0-7]+)/ : null
    var radix = number == 'decimal'
      ? 10
      : number == 'hex' ? 16 : number == 'octal' ? 8 : null
    var numPart = [],
      textPart = []
    if (number) {
      for (var i = 0; i < text.length; i++) {
  ...
  if (
    ...
    cm.operation(function() {
      cm.ace.forEachSelection(function() {
        ...
        cm.state.vim.lastHPos = sel.$desiredColumn == null
          ? sel.lead.column
          : sel.$desiredColumn
        var head = cm.getCursor('head')
        var anchor = cm.getCursor('anchor')
        ...
        sel.$desiredColumn = cm.state.vim.lastHPos == -1
          ? null
