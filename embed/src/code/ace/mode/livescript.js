...
import * as $_____mode_matching_brace_outdent from '../mode/matching_brace_outdent'
import * as $_____mode_text from '../mode/text'
var identifier, LiveScriptMode, keywordend, stringfill
identifier =
  '(?![\\d\\s])[$\\w\\xAA-\\uFFDC](?:(?!\\s)[$\\w\\xAA-\\uFFDC]|-[A-Za-z])*'
export var Mode = (LiveScriptMode = (function(superclass) {
  var indenter,
    prototype = extend$(
      (
  ...
  function LiveScriptMode() {
    var that
    this.$tokenizer = new $_____tokenizer.Tokenizer(LiveScriptMode.Rules)
    if ((that = $_____mode_matching_brace_outdent)) {
